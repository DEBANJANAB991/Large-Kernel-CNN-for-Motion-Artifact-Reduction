#!/usr/bin/env python3
"""
Extended evaluation: PSNR, SSIM, MAE, RMSE, VIF, LPIPS
Computed on reconstructed CT volumes — no GPU required.
Run AFTER fdk_reconstruction.py has generated all volumes.
"""
...
try:
    ...
    print("sewar not installed. Run: pip install sewar")
    VIF_AVAILABLE = False
...
def normalize_pair(clean_vol: np.ndarray, pred_vol: np.ndarray):
    """Normalizes both using clean statistics."""
    v_min = clean_vol.min()
    v_max = clean_vol.max()
...
def get_lpips_model():
    if not LPIPS_AVAILABLE:
        return None
    # net='alex' is faster than 'vgg', runs fine on CPU
...
def compute_lpips(lpips_model, img1: np.ndarray, img2: np.ndarray) -> float:
    ...
    def to_tensor(img):
        ...
        img_t = img_t.repeat(1, 3, 1, 1)          # (1, 3, H, W) — grayscale to RGB
...
def compute_all_metrics(
    ...
    for z in range(clean_norm.shape[0]):
        ...
        # LPIPS — skip some slices to save time (every 5th)
        if z % 5 == 0:
...
def main():
    ...
    parser.add_argument("--clean-folder", default="clean_test")
    ...
    # Load LPIPS model once — reused across all patients
    lpips_model = None

...
def fix_replk_input(x, y):
    _, _, H, W = x.shape

    # Make divisible by 4 (safe for encoder-decoder)
    H_new = (H // 4) * 4
    W_new = (W // 4) * 4
...
class Sinogram2DDataset(Dataset):
    ...
    def __getitem__(self, idx):
      ...
      #  art   = art.T
       ...
       # art_t, clean_t = crop_detector_only(art_t, clean_t, size=128)
...
class RestormerWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        # 🔥 enforce divisible by 8 INSIDE model
        H_new = (H // 8) * 8
        W_new = (W // 8) * 8
...
def parse_args():
    ...
    p.add_argument("--loss", choices=["l1", "l1_ssim"], default="l1_ssim")
...
def _model_dir(ckpt_root: Path, model_name: str, loss_name: str) -> Path:
    key = model_name.lower()
    folder = _FOLDER_MAP.get(key, key)

    if key == "mr_lkv":
        if loss_name == "l1":
            return Path(ckpt_root) / f"{folder}_l1"
        else: 
...
def main():
    ...
    model_ckpt_dir = _model_dir(args.ckpt_dir, args.model, args.loss)
    ...
    elif args.model == "swinir": #embed_dim=64 or 96 or 180+, depth =[4,4,4,4](if time, train again)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for art, clean in train_loader:
            if args.model in ["restormer", "swinir"]:
                art, clean = fix_restormer_input(art, clean)
            if args.model == "replk":
                art, clean = fix_replk_input(art, clean)
